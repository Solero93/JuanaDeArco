...
public class JuanaDeArco implements KeyListener {
    ...
    private void actualitzarJoc() {
        ...
        if (heroina.isMort() || heroina.haTrobatBrayan()) {
...
// Inicialitzem de nou al joc, perqué ni l'escut ni brayan no s'hagi trobat        
        heroina.setHaTrobatBrayan(false);
        heroina.setHaTrobatEscut(false);
...
// Fixem les condicions de final de joc per cas de 
        // trobar Brayan i no l'escut
        // i per trobar brayan i l'escut també
        if (heroina.haTrobatBrayan() && !(heroina.haTrobatEscut())) {
	    g2.setFont(f);
            g2.setColor(Color.white);
            g2.drawString("GAME OVER", 160, 90);
            g2.drawString("Estàs mort!", 160, 140); 
            g2.drawString("Has trobat el General Brayan.", 160, 90);
            g2.drawString("Un altre dia potser ...", 160, 90);
            f = new Font("Dialog", Font.PLAIN, 16);
        }
        if (heroina.haTrobatBrayan() && (heroina.haTrobatEscut())) {
            g2.setFont(f);
            g2.setColor(Color.white);
            g2.drawString("T'has pogut defensar del General Brayan!", 160, 90);
            f = new Font("Dialog", Font.PLAIN, 16);
        }
        f = new Font("Dialog", Font.PLAIN, 16);
        g2.setFont(f);
